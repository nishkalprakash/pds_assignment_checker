...
for question in Path().glob("*/"):
    ...
    mq = assign_folder_name / question / f"{assign_folder_name}_{question}_moss_results.txt"
    ...
    for f in chain(pds_folder_name.glob("*.[cC]"), pds_folder_name.glob("*.txt")):
        ...
        new_fname = f"{mapping[name]} - {name} - {}}.c"
...
moss_out = Path() / f"{A_.format(a=a)}_moss_results.txt"
