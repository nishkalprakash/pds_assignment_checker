...
1.	a) Read any four points  (x1,y1, x2, y2, x3, y3, x4, y4)  in 2D Euclidean space.
    b) Print the three points that constitute the smallest triangle (based on the area of the triangle).
    c) Print the area of the smallest triangle
...
int main() {
    ...
    float a, b, c, area1,area2,area3,area4, min_area;
    int i, j, k;
    ...
    //find the minimum area and print the points having minimum area
    min_area = area1;
    float min_param = s1;
    if (area2 < min_area) {
        min_area = area2;
    ...
    if (min_area == area1) {
        printf("The points (%f,%f), (%f,%f), (%f,%f) form a triangle with minimum area.\n", x1, y1, x2, y2, x3, y3);
    }
    else if (min_area == area2) {
        printf("The points (%f,%f), (%f,%f), (%f,%f) form a triangle with minimum area.\n", x1, y1, x2, y2, x4, y4);
    }
    else if (min_area == area3) {
        printf("The points (%f,%f), (%f,%f), (%f,%f) form a triangle with minimum area.\n", x1, y1, x3, y3, x4, y4);
    }
    else if (min_area == area4) {
        printf("The points (%f,%f), (%f,%f), (%f,%f) form a triangle with minimum area.\n", x2, y2, x3, y3, x4, y4);
    }

    // print the minimum area
    printf("The minimum area is %f.\n", min_area);

    if(min_area==0){
        min_param = 0;
    }

    // print the perimeter of traingle with minimum area
    // printf("The perimeter of the triangle with minimum area is %f.\n", 2*min_param);
