...
// and right Brackets
bool is Matching Pair (char character1, char character2)
{
...
bool are Brackets Balanced (char exp[])
{
 int i = 0;
...
// Pop the top element from stack, if it is not
 ...
 else if (! is Matching Pair(pop(&stack), exp[i]))
 return 0;
 }
 i++;
 }
...
void push(struct sNode** top_ref, int new_data)
{
 ...
 = (struct sNode*) malloc(size of(struct sNode));
 if (new_node == NULL) {
