...
polynomial foo(char p1[])
{
    ...
    for (i = 0; p1[i] != '\0'; i++)
    {
        ...
        {
            info_p1.coefficients[k] = 1;
            k++;
            if (p1[i + 1] == '^')
            {
                info_p1.exponents[l] = p1[i + 2];
                l++;
            }
            else
            {
                info_p1.exponents[l] = 1;
            }
        }
        else
        {
            info_p1.coefficients[k] = p1[i - 1];
            k++;
            if (p1[i + 1] == '^')
            {
                info_p1.exponents[l] = p1[i + 2];
    ...
    {
        info_p1.exponents[l] += p1[j] * pow(10, n);
        n++;
    }
    return (info_p1);
}
...
int main()
{
    ...
    //  function call
    P2 = foo(p2);
    resultant = multiplication(P1, P2);
