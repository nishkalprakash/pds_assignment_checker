...
int enque(int A[],int len, int n)
{
    ...
    if(flag==1){
        if(len==MAX) printf("Queue reached its maximum size");
        else{
            A[len]=n;
            len++;
...
int main()
{
    ...
    printf("\nEnter the option: 1. Enque, 2. Deque, 3. End queue\n");
    scanf("%d",&op);
    while (1>0){
        if (op==1){
            printf("Enter the element : ");
        ...
        else printf("Enter a valid option!"); //when user gives input other than "1","2","3"
        scanf("%d",&op);
    }
    printf("Exited!");
}
