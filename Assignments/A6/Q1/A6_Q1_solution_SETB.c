...
y = ∑_(k=1)^x〖k^2/(2k+1)  -〖(-1)〗^k.k〗  + sin(x/3)
	...
	If y is smaller than any value in Filter, store x into Bin_A. 
	If y is greater than any value in Filter, store x into Bin_B.
...
Note: 
	...
	For the calculation of sin(x), include math.h and use the sin() function.
...
int main() {
    ...
    while(1) {
        ...
        if (y < maxFilter)              Bin_A[A_len++] = x;
        if (y > minFilter)              Bin_B[B_len++] = x;
...
/* Sample Output:
Filter: [1367.87, 1.00, 1154.15, 813.15, 1680.27, 1520.76, 72.04, 507.65, 15.14, 1154.15]

Bin_A: [10, 51, 72, 37]

Bin_B: [10, 51, 72, 100, 37]
*/