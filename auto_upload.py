...
def upload_to_moodle(a, ql,s=None,roll=None):
    ...
    xp="descendant::div[contains(concat(' ',normalize-space(@class),' '),' comment ')]//%s[contains(concat(' ',normalize-space(@id),' '),'%s')]"
    ...
    for q_topic in q_topics:
        ...
        # try:
            ...
            if s and r not in std_dict_set[s]:
                continue
            cm=next_div.find_element(by=By.XPATH,value='descendant::iframe')
        ...
        # for i,elem in enumerate(nrt):
            ...
            # cm.click()
            insert_cm(driver,cm, f"{s_m_c[r]['c']}")
        ...
        driver.find_element(By.XPATH, value="//input[@type='submit' and @value='Save and show next']").click()
        sleep(2)
