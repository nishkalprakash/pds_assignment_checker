...
from lib.pds_globals import A_, A_PATH_, A_Q_PATH_, BASE, HOME, Q_, VAR
...
# a='4'
out = Path(f"{A_PATH_.format(a=a)}/{A_.format(a=a)}_submissions.csv")
...
if not out.exists():
    ...
    for q in q_list:
        out_q=Path(f"{A_Q_PATH_.format(a=a,q=q)}/{Q_.format(q=q)}_submissions.csv")
        if out_q.exists():
        ...
        for tr in t_elems:
            ...
            # table.append()
            std_name=row[2].find_element(by='tag name',value='a').get_attribute('innerHTML').strip()
            if std_name:
                val = (
                    f"Question_{q_topic['q']} Submitted on: {datetime.strptime(t,'%A, %d %B %Y, %I:%M %p').strftime('%I:%M %p')}"
                    if (t:=row[8].text.strip()) and row[9].text
                    else f"Question_{q_topic['q']} Not Submitted"
                )
                try:
                    std_time_dict[map_name_to_roll[std_name]].extend([val])
                except:
                    std_time_dict[map_name_to_roll[std_name]] = [val]
                    
                q_sub_dict[map_name_to_roll[std_name]]=val
        # tables.append(table)
